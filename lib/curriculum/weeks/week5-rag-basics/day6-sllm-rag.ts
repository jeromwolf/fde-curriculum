...
## ⚠️ 실제 사례: 왜 기업들이 로컬 LLM을 선택하는가?

### 삼성전자 ChatGPT 사용 금지 사례 (2023)

\`\`\`
[실제 사건]
삼성전자 반도체 부문에서 ChatGPT 사용을 금지

이유:
- 엔지니어가 소스코드를 ChatGPT에 붙여넣어 디버깅 요청
- 회의록을 ChatGPT로 요약 → 내부 전략 정보 유출 가능성
- 이후 삼성전자는 사내 AI 서비스 개발 방향으로 전환

교훈:
"회사 데이터를 외부 서버로 보내도 괜찮은가?"
→ 로컬 LLM은 이 고민을 근본적으로 해결
\`\`\`
...
| 업무 | 민감 데이터 | 법적 근거 |
|------|------------|-----------|
| **인사/급여 처리** | 연봉, 평가, 개인정보 | 개인정보보호법 |
| **의료/건강 정보** | 진료 기록, 처방전 | 의료법, 민감정보 |
| **금융 거래 분석** | 계좌, 거래내역 | 금융실명법 |
| **법률 문서 검토** | 계약서, 소송자료 | 비밀유지의무 |
| **내부 전략 문서** | 사업계획, M&A | 영업비밀보호법 |

> 💡 **핵심 질문**: "이 데이터가 외부로 나가도 괜찮은가?"
> → NO라면 로컬 LLM 필수
...
\`\`\`
         ...
         ┌────┴────┐              ┌─────┴─────┐
         │         │              │           │
     복잡한 추론?  단순 요약?    일회성?    반복적?
         │         │              │           │
    Cloud API   Local LLM    Cloud API    Local LLM
    (GPT-4)    (Llama 8B)    (빠른응답)   (비용절감)
\`\`\`

> **실무 팁**: "민감 문서는 로컬, 일반 질문은 클라우드"
...
### 4. [다국어 모델] Qwen 모델이 중국어로 답변하는 문제

\`\`\`python
# ❌ 잘못된 예시: 한국어 문서로 RAG 했는데 중국어로 답변
prompt = f"""
...
질문: {question}
"""
# 🔴 Qwen2.5는 다국어 모델이라 중국어로 답변할 수 있음!

# ✅ 올바른 예시: 언어를 명시적으로 지정
prompt = f"""
당신은 한국어 연말정산 전문 상담사입니다.
반드시 한국어로만 답변하세요.
절대 중국어나 영어로 답변하지 마세요.
...
**기억할 점**: Qwen, Llama 등 다국어 모델은 **언어 명시 필수**. 특히 Qwen은 중국어가 기본값처럼 동작할 수 있음.

> 💡 **실무 팁**: 프롬프트 첫 줄에 "반드시 한국어로만 답변하세요"를 추가하면 99% 해결됩니다.
`,
      keyPoints: [
