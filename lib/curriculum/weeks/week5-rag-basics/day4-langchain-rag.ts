...
프롬프트가 RAG 답변 품질의 **50%를 결정**합니다. 4가지 핵심 요소를 마스터하세요.
...
### 2. 명확한 지시 (Instruction)

\`\`\`
❌ "잘 답변해주세요"
✅ "금액, 비율 등 구체적 수치를 포함해서 답변하세요"
✅ "단계별로 설명하고 예시를 포함하세요"
\`\`\`
...
\`\`\`python
prompt = ChatPromptTemplate.from_template("""
당신은 연말정산 전문 상담사입니다.  # 역할

[답변 규칙]
- 금액, 비율, 조건 등 구체적 수치를 포함하세요  # 지시
- 반드시 한국어로만 답변하세요  # 제약 조건
- 중국어나 다른 언어를 절대 사용하지 마세요  # 제약 조건
- 문서에서 찾을 수 없으면 솔직히 모른다고 말하세요  # 제약 조건
- 번호나 목록으로 구조화하여 답변하세요  # 출력 형식

[하지 말아야 할 것 - 네거티브 프롬프트]
- 문서에 없는 내용을 추측하지 마세요
- 불확실한 정보를 확신하여 말하지 마세요
- 개인적인 의견을 제시하지 마세요
...
- [ ] 제약 조건이 있는가?
- [ ] 출력 형식을 지정했는가?
- [ ] 언어를 명시했는가? (다국어 모델 사용 시 필수)
- [ ] 네거티브 프롬프트가 있는가?

> 💡 **팁**: 프롬프트는 외부 파일(\`prompts/system_prompt.txt\`)로 분리하면 코드 수정 없이 즉시 튜닝 가능!
      `,
      keyPoints: [
        '🔁 with_retry()로 재시도 로직 구현',
        '⚡ RunnableParallel로 소스와 컨텍스트 동시 추출',
        '📦 batch()로 배치 처리, stream()으로 스트리밍',
        '✍️ 프롬프트 4요소: 역할/지시/제약/형식',
      ],
